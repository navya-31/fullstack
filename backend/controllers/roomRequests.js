...
export const getRoomRequestsByRoomId = async (req, res) => {
  ...
  try {
    const roomRequests = await roomRequestModel
      .find({ room_id: id })
      .sort({ date_issued: 1 });
    console.log(roomRequests);
    res.status(200).json(roomRequests);
...
export const createRoomRequest = async (req, res) => {
  ...
  try {
    ...
    const updatedUser = await tenantModel.updateOne(
      { user_id: uid },
      { $set: { has_booked: true } }
    );
...
export const updateRoomRequest = async (req, res) => {
  ...
  const updatedRoomRequest = await roomRequestModel.findByIdAndUpdate(
    _id,
    { ...roomRequest, _id },
    { new: true }
  );
